...
\label{sec: Idea gen}
Idea generation represents a pivotal stage in the scientific process, corresponding to the methodology design phase, where researchers synthesize knowledge to propose innovative approaches addressing existing challenges. To systematically evaluate the quality and relevance of generated ideas, we establish complementary metrics encompassing novelty, professionalism, completeness, rigor, feasibility, length conformity (penalizing overly brief or verbose outputs), and semantic repetition (penalizing redundant expressions).

In real-world scientific workflows, idea generation directly maps to the methodology design stage following the literature review. At this stage, scientists integrate prior findings with creative reasoning to develop novel hypotheses and methodological frameworks aimed at resolving identified knowledge gaps or limitations.
...
To better characterize the scope of dry experiments, we categorize representative computational functions commonly encountered across disciplines, including numerical integration, differential equation solving, data preprocessing, and experimental result visualization. The completion or generation of these functions offers a rigorous measure of how well AI systems can operationalize scientific intent into executable form.
...
Scientific experimental reasoning in this benchmark focuses on multi-modal scientific reasoning, requiring models to interpret heterogeneous information drawn from diverse sources. We consider five representative modalities: (1) process images that integrate symbolic and textual information to depict workflows or variable relationships; (2) observation images representing raw data captured by instruments such as telescopes, satellites, or microscopes; (3) experiment images documenting laboratory setups and procedures; (4) simulation images generated by computational models to visualize physical or chemical processes; and (5) visualization images such as plots or charts that reveal patterns within structured datasets. Collectively, these modalities reflect the multi-faceted and evidence-driven nature of scientific inquiry.
...
To reason effectively over such diverse inputs, we define four complementary reasoning paradigms: (1) signal perception, focusing on the extraction of direct patterns from visual signals; (2) attribute understanding, which demands domain knowledge to interpret key visual or contextual features; (3) comparative reasoning, involving integration and comparison across multiple sources to ensure consistency and rigor; and (4) causal reasoning, aimed at uncovering underlying mechanisms and scientific principles. These paradigms collectively span the hierarchy from low-level perception to high-level scientific inference.
...
\subsection{Benchmark Data Distribution}
